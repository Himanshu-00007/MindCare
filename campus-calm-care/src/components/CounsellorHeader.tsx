...
const CounsellorHeader = () => {
  ...
  useEffect(() => {
    const fetchCounsellorName = async () => {
      const token = localStorage.getItem("Token");
      if (!token) return;
      try {
        const res = await axios.get("https://mindcare-lf3g.onrender.com/api/v1/counsellors/me", {
          headers: { Authorization: `Bearer ${token}` },
        });
        setCounsellorName(res.data.name);
      } catch (err) {
        console.error(err);
      }
    };
  ...
  const menuItems = [
    { icon: Activity, label: "Dashboard", path: "/counsellor-dashboard" },
  ];
      ...
      <div className="container mx-auto px-6 py-4 flex items-center justify-between">
        ...
        >
          ...
          <div>
            ...
            <h2 className="text-lg font-semibold text-gray-900">{counsellorName || "Counsellor"}</h2>
          </div>
        </motion.div>
