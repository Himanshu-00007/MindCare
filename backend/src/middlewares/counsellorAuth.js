...
export const verifyJWT = async (req, res, next) => {
  try {
    ...
    const user = await Counsellor.findById(decoded._id).select("-password -refreshToken");
    if (!user) return res.status(401).json({ message: "Invalid access token" });

    req.user = { ...user.toObject(), role: "counsellor" };
    next();
  } catch (err) {
    console.error("Counsellor JWT error:", err);
